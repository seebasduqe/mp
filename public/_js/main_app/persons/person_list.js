...
$(document).ready(function () {
    ...
    oTable_persons = $('#personTbl').DataTable({
        "orderCellsTop": true,
        "stateSave": true,
        "scrollX": true,
        "scrollY": 600,
        ...
        "columnDefs": [
            ...
            { "targets": 2, "visible": true, "searchable": true, "bSortable": true, "sClass": "", "width": "138px" },//surnames
            { "targets": 3, "visible": true, "searchable": true, "bSortable": true, "sClass": "", "width": "138px" },//nif
            { "targets": 4, "visible": true, "searchable": false, "bSortable": true, "sClass": "", "width": "138px" },//telephone
            { "targets": 5, "visible": true, "searchable": false, "bSortable": true, "sClass": "", "width": "138px" },//email
            { "targets": 6, "visible": true, "searchable": false, "bSortable": false, "sClass": "", "width": "70px" }
        ],
            ...
            $(".filters th").each(function (i) {
                if (filter_columns.includes(i)) {
                    var select = $('<select  class="form-control"><option value=""></option></select>')
                        .appendTo($(this).empty())
                        .on('change', function () {
                            oTable_persons.column(i)
                                .search($(this).val())
                                .draw();
                        });

                    oTable_persons.column(i).data().unique().sort().each(function (d, j) {
    ...
    // delete person ajax
    $(document).on('click', '.delete_person', function (e) {


        let person_id = $(this).attr('data-attr');

        $.confirm({
            title: 'Eliminar Persona',
            content: '¿Estas seguro que quieres eliminar la persona?',
            buttons: {
                cancelar: {
                    text: 'Cancelar',
                    btnClass: 'btn-red',
